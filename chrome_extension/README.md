...
The **Learning** tab lists reviewable feedback candidates by Preferences, Evidence, Skill Candidates, and Policy Issues. You can submit explicit feedback, inspect its source events and conflicts, then review the candidate. Confirming a preference or career fact is an explicit action. An approved Skill Candidate can be staged, inspected, evaluated against the server-owned synthetic dataset, and explicitly published. Published versions remain inactive until a separate activation action. The panel shows validation, hard gates, forward tests, version history, basic monitoring, and explicit rollback. A canary requires an explicitly created test Session (`test_canary: true` in the API); the panel does not yet expose this toggle. Shadow mode records matching version selection without changing model context or running a second model comparison. The extension does not cache feedback, candidates, or Skill packages; it reloads them from the local server.
